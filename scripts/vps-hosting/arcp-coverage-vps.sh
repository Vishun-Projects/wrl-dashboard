...
REMOTE_SCRIPT="${REMOTE_DIR}/arcp-coverage-remote.sh"

echo "==> Syncing ARCP coverage helper to ${VPS_HOST}:${REMOTE_SCRIPT}"
ssh "$VPS_HOST" "mkdir -p '${REMOTE_DIR}'"
scp "${ROOT}/scripts/vps-hosting/arcp-coverage-remote.sh" "${VPS_HOST}:${REMOTE_SCRIPT}"
ssh "$VPS_HOST" "chmod +x '${REMOTE_SCRIPT}'"

echo "==> Running ARCP coverage check on VPS..."
