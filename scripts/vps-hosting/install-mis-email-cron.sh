#!/usr/bin/env bash
# Install MIS email digest cron (every 15 min IST) on VPS.
...
CRON_LINE="*/15 * * * * ${INSTALL_ROOT}/scripts/vps-hosting/mis-email-digest.sh >> ${INSTALL_ROOT}/logs/mis-email-cron.log 2>&1"
...
if [[ "${1:-}" == "--local" ]]; then
  ...
  echo "==> Installed MIS email cron (every 15 min IST)"
...
line="*/15 * * * * ${root}/scripts/vps-hosting/mis-email-digest.sh >> ${root}/logs/mis-email-cron.log 2>&1"
(crontab -l 2>/dev/null | grep -v 'mis-email-digest.sh' || true
 echo "CRON_TZ=Asia/Kolkata"
 echo "$line") | crontab -
echo "==> MIS email cron installed (every 15 min IST)"
crontab -l | grep -E 'CRON_TZ|mis-email' || true
REMOTE
