...
} from '@/lib/arcp-claims-crm-labels';
import {
...
async function loadArcpAggregatesCoverageAware(
  ...
  if (!startDate || !endDate) {
    ...
    const grandTotals = await queryArcpClaimsGrandTotals(opts);
...
export async function loadArcpClaimsAggregatesHybrid(
  ...
  if (!readArcpFromPostgres()) {
    ...
    return { aggregates, source: 'crm', grandTotals };
  }
  ...
  if (!readiness.ready || readiness.rowCount === 0) {
    ...
    return { aggregates, source: 'crm_fallback', grandTotals };
  }
