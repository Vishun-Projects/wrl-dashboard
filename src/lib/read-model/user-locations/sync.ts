...
import { fetchCrmUserLocations } from './crm-fetch';
import { upsertUserLocations } from './upsert';

export const USER_LOCATIONS_ENTITY = 'crm_user_locations';

export type UserLocationsSyncResult = {
  ok: boolean;
  dateFrom: string;
  dateTo: string;
  fetched: number;
...
function defaultStart(): string {
  return process.env.USER_LOCATION_START_DATE?.trim() || `${new Date().getFullYear()}-01-01`;
}
  ...
  await withClient(async (client) => {
    await client.query(
      `UPDATE sync_state
       ...
       WHERE entity = $1`,
      [USER_LOCATIONS_ENTITY, watermark, rowsUpserted]
    );
  });
}

export async function runUserLocationsSync(opts?: {
  dateFrom?: string;
  dateTo?: string;
}): Promise<UserLocationsSyncResult> {
  ...
  console.log(`[user-locations] sync ${dateFrom} .. ${dateTo}`);
  const chunks = splitDateRangeByDays(dateFrom, dateTo, 1);
  let fetched = 0;
  let upserted = 0;

  for (const chunk of chunks) {
    const rows = await fetchCrmUserLocations(chunk.start, chunk.end);
    fetched += rows.length;
    const wrote = await withClient((client: pg.PoolClient) => upsertUserLocations(client, rows));
    upserted += wrote;
