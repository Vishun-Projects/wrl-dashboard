...
export type { ArcpChunkLoadMeta } from '@/features/arcp/server/chunk-cache';
...
} from '@/lib/read-model/arcp/crm-fetch';
...
async function loadAggregateChunkCached(
  ...
  try {
    ...
    if (hit && hit.payload.kind === 'agg') {
      if (trackJob) {
        const { markChunkDone } = await import('@/features/arcp/server/load-job');
    ...
    if (trackJob) {
      const { markChunkFailed } = await import('@/features/arcp/server/load-job');
