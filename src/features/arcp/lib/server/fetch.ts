...
export type { ArcpChunkLoadMeta } from '@/features/arcp/lib/server/chunk-cache';
...
async function loadAggregateChunkCached(
  ...
  try {
    ...
    if (hit && hit.payload.kind === 'agg') {
      if (trackJob) {
        const { markChunkDone } = await import('@/features/arcp/lib/server/load-job');
    ...
    if (trackJob) {
      const { markChunkFailed } = await import('@/features/arcp/lib/server/load-job');
