import type { RegisterSummary, RegisterSummaryBucket } from '@/features/report/services/search';
import type { BranchSummaryRow } from '@/lib/summary/derive';
import { openReportsDb } from '@/features/report/services/corpus-storage';
import {
  accountRowScore,
...
/** Summary/Accounts API rows include region hierarchy and headcount. */
export function isApiShapedSummary(rows: unknown[]): boolean {
  return (
    rows.length > 0 &&
    rows.some(
      (r) =>
