...
export function resolveMisEmailIntroText(
  preset?: MisEmailIntroPreset | null,
  orgIntros?: { normal?: string; revised?: string } | null
): string {
  const key = preset === 'revised' ? 'revised' : 'normal';
  const fromOrg = key === 'revised' ? orgIntros?.revised : orgIntros?.normal;
  if (fromOrg?.trim()) return fromOrg.trim();
  return MIS_EMAIL_INTRO_BY_PRESET[key];
}

export function resolveMisEmailSubjectTemplate(
  preset?: MisEmailIntroPreset | null,
  orgSubjects?: { normal?: string; revised?: string } | null
): string {
  const key = preset === 'revised' ? 'revised' : 'normal';
  const fromOrg = key === 'revised' ? orgSubjects?.revised : orgSubjects?.normal;
  if (fromOrg?.trim()) return fromOrg.trim();
  return key === 'revised'
    ? 'Daily MIS Report as on {asOn} (Revised)'
    : 'Daily MIS Report as on {asOn}';
}
...
export type MisEmailTemplateBranding = {
  ...
  introText?: string;
  /** Drives brand subtitle Revised labeling when set. */
