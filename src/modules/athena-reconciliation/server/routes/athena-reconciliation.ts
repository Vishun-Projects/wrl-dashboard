...
} from '@/modules/athena-reconciliation/server/metrics';
import {
  fetchAthenaFailedCallsRows,
  generateAthenaReconciliationCsv,
} from '@/modules/athena-reconciliation/server/rows';
import type {
...
function parseFilterParams(searchParams: URLSearchParams): AthenaReconciliationFilterParams {
  ...
  const status = validStatuses.includes(statusParam as AthenaReconciliationStatus | 'ALL')
  ...
  return {
    ...
    branches: parseListParam(searchParams, 'branch', 'branches'),
    clients: parseListParam(searchParams, 'client', 'clients'),
    callTypes: parseListParam(searchParams, 'callType', 'callTypes'),
    failureReasons: parseListParam(searchParams, 'failureReason', 'failureReasons'),
    status,
    search: searchParams.get('search') || null,
    excludedReasons: parseListParam(searchParams, 'excludedReasons'),
    treatAsRegisteredReasons: parseListParam(searchParams, 'treatAsRegisteredReasons'),
...
export async function POST(req: NextRequest) {
  try {
    ...
    let body: { action?: string; dateFrom?: string; dateTo?: string; reprocessAll?: boolean; fullBackfill?: boolean } = {};
    try {
      body = await req.json();
    } catch {
      // Empty body is valid
