...
import { fetchArcpIncrementalRows } from '@/modules/arcp/server/sync/crm-fetch';
import {
  ARCP_ENTITY,
...
} from '@/modules/arcp/server/sync/lock';
import { dedupeArcpRows, maxArcpWatermarks, processArcpRows } from '@/modules/arcp/server/sync/transform';
import { invalidateArcpPostgresCoverageCache } from '@/modules/arcp/server/sync/coverage-query';
import { countArcpRows, upsertArcpRows } from '@/modules/arcp/server/sync/upsert';
...
export async function runArcpIncrementalSync(): Promise<ArcpIncrementalResult> {
  ...
  let prep = await withClient(async (client) => {
    await import('@/modules/arcp/server/sync/lock').then((m) => m.releaseStaleArcpSyncLock(client));
