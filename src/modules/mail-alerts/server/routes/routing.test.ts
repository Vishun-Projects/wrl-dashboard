...
describe('mis-email-routing API auth', () => {
  ...
  it('returns 401 when no request user', async () => {
    ...
    const { GET } = await import('@/modules/mail-alerts/server/routes/routing');
  ...
  it('forwards clientSourceMode on create', async () => {
    ...
    const { POST } = await import('@/modules/mail-alerts/server/routes/routing');
  ...
  it('forwards clientSourceMode on update', async () => {
    ...
    const { PUT } = await import('@/modules/mail-alerts/server/routes/routing');
    const res = await PUT(
      new Request('http://localhost/api/admin/mis-email-routing', {
        method: 'PUT',
        headers: { Origin: 'http://localhost' },
