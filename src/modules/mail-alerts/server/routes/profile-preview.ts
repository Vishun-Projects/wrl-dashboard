...
import { previewMisEmailCompose } from '@/modules/mail-alerts/services/compose-digest';
import { parseMisEmailIntroPreset } from '@/modules/mail-alerts/services/email-template';
import {
  ...
  try {
    ...
    if (!recipient.includeSummary && !recipient.includeDetailed && !recipient.includeKeyAccount) {
      return NextResponse.json(
        {
          error:
            'Mail access alone is not enough — also assign MIS Summary, Call Register, or Key Account access.',
        },
        { status: 403 }
      );
    }
