...
} from '@/modules/mail-alerts/services/email-body-layout';
import {
...
import { clientAccountDisplayName } from '@/modules/mis';
...
export function buildMisEmailSkeletonPreview(params: {
  ...
  const letter = params.letterCopy;
  const portalUrl =
    params.portalUrl?.replace(/\/$/, '') ||
    letter?.portalBaseUrl?.replace(/\/$/, '') ||
    '/report';

  const subjectTemplate = resolveMisEmailSubjectTemplate(params.introPreset, {
    normal: letter?.subjectTemplate,
    revised: letter?.subjectTemplateRevised,
  });
  const introText = resolveMisEmailIntroText(params.introPreset, {
    normal: letter?.introTextNormal,
    revised: letter?.introTextRevised,
  });
  ...
  const html = buildDigestEmailHtml({
    ...
    branding: {
      ...
      brandTitle: letter?.brandTitle,
      brandSubtitle: resolveMisEmailBrandSubtitle(letter?.brandSubtitle, params.introPreset),
      subjectTemplate,
      introText,
