...
import { sendMisEmailComposeBatch } from '@/modules/mail-alerts/services/compose-digest';
import { loadDigestRecipientById } from '@/modules/mail-alerts/services/recipients';
import type { MisEmailPreferences } from '@/modules/mail-alerts/services/preferences';
import { runMisEmailDigest, runMisEmailTestBatch } from '@/modules/mail-alerts/services/run-digest';
...
async function main(): Promise<void> {
  ...
  try {
    switch (command) {
      ...
      default:
        console.log(`Usage:
  npx tsx src/modules/mail-alerts/services/cli.ts digest
  npx tsx src/modules/mail-alerts/services/cli.ts test [--user=<uuid>] [--to=a@x.com,b@y.com] [--cc=c@z.com]
  npx tsx src/modules/mail-alerts/services/cli.ts send-user   (MIS_EMAIL_SEND_PAYLOAD=base64 json)
