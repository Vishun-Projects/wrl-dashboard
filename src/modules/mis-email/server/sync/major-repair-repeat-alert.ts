...
import {
  ...
  priorityRefreshHotFromCrm,
} from '@/lib/read-model/priority-refresh-trns';
...
/** TRNs to force-refresh in hot after a successful major-repair alert send. */
export function majorRepairAlertPriorityTrns(params: {
  triggerTrn: string;
  detailTrns: string[];
  serialRowTrns: string[];
}): string[] {
  return normalizePriorityTrns([
    params.triggerTrn,
    ...params.detailTrns,
    ...params.serialRowTrns,
  ]);
}
...
export async function checkMajorRepairRepeatAlerts(upsertedRows: HotRow[]): Promise<void> {
  ...
  for (const [serial, rowsForSerial] of bySerial) {
    ...
    try {
      ...
      const priorityTrns = majorRepairAlertPriorityTrns({
        triggerTrn: trigger.vtrnno,
        detailTrns: details.map((d) => d.vtrnno),
        serialRowTrns: rowsForSerial.map((r) => r.vtrnno),
      });
      try {
        await priorityRefreshHotFromCrm(priorityTrns);
      } catch (refreshErr) {
        console.warn(
          `[sync-worker] major-repair-repeat-alert: priority hot refresh failed for ${trigger.vtrnno}:`,
