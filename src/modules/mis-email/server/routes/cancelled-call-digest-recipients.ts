...
import {
  ...
  type CancelledCallDigestRecipient,
...
function recipientSnapshot(r: CancelledCallDigestRecipient) {
  return {
    branch: r.branch,
...
async function requireAccess(request: Request) {
  ...
  if (!user) {
    await logAccessDenied({
      request,
      statusCode: 401,
      reason: 'cancelled_call_digest_recipients_unauthorized',
    });
  ...
  if (!canAccessPage(auth.permissions, 'cancelled_call_alerts')) {
    await logAccessDenied({
      ...
      statusCode: 403,
      reason: 'cancelled_call_digest_recipients_forbidden',
    });
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const recipient = await createCancelledCallDigestRecipient({
    ...
    await logAction({
      request,
      action: 'admin.cancelled_call_digest_recipient.create',
      actor: access.actor,
      result: 'success',
      statusCode: 201,
      target: {
        type: 'cancelled_call_digest_recipient',
        id: String(recipient.id ?? ''),
        label: recipient.email,
      },
      summary: `Created cancelled-call digest recipient ${recipient.email}`,
      metadata: {
    ...
    await logAction({
      ...
      result: 'failure',
      statusCode: 400,
      summary: 'Failed to create cancelled-call digest recipient',
      metadata: { message },
    });
    return NextResponse.json(
      { error: safeErrorMessage(err, 'Failed to create recipient') },
      { status: 400 }
    );
  }
}
...
export async function PUT(request: Request) {
  ...
  try {
    ...
    const before = await getCancelledCallDigestRecipient(id);
    const recipient = await updateCancelledCallDigestRecipient({
      id,
      branch: String(body.branch ?? ''),
      recipientName: String(body.recipientName ?? ''),
      email: String(body.email ?? ''),
    ...
    await logAction({
      request,
      action: 'admin.cancelled_call_digest_recipient.update',
      actor: access.actor,
      result: 'success',
      statusCode: 200,
      target: {
      ...
      summary: `Updated cancelled-call digest recipient ${recipient.email}`,
      metadata: {
        before: beforeSnap,
        after: afterSnap,
        changes,
    ...
    await logAction({
      ...
      result: 'failure',
      statusCode: 400,
      summary: 'Failed to update cancelled-call digest recipient',
      metadata: { message },
    });
    return NextResponse.json(
      { error: safeErrorMessage(err, 'Failed to update recipient') },
      { status: 400 }
    );
  }
}
...
export async function DELETE(request: Request) {
  ...
  try {
    ...
    const before = await getCancelledCallDigestRecipient(id);
    await deleteCancelledCallDigestRecipient(id);
    const beforeSnap = before ? recipientSnapshot(before) : null;
    await logAction({
      request,
      action: 'admin.cancelled_call_digest_recipient.delete',
      actor: access.actor,
      result: 'success',
      statusCode: 200,
      target: {
      ...
      summary: beforeSnap
        ? `Deleted cancelled-call digest recipient ${beforeSnap.email} (${beforeSnap.branch})`
        : `Deleted cancelled-call digest recipient ${id}`,
      metadata: {
        before: beforeSnap,
      },
    });
    ...
    await logAction({
      ...
      result: 'failure',
      statusCode: 400,
      summary: 'Failed to delete cancelled-call digest recipient',
      metadata: { message },
    });
    return NextResponse.json(
      { error: safeErrorMessage(err, 'Failed to delete recipient') },
      { status: 400 }
    );
  }
}
