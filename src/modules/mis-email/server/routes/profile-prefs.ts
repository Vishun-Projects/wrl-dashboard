...
import { resolveAvailableBodySections } from '@/modules/mis-email/services/body-sections';
...
export async function GET(request: Request) {
  ...
  try {
    ...
    const { toMisEmailLetterCopy } = await import(
      '@/modules/mis-email/services/org-settings-defaults'
    );
...
export async function PATCH(request: Request) {
  ...
  try {
    ...
    const prefsAuditSlice = (p: MisEmailPreferences) => ({
      subscribed: p.subscribed ?? null,
      sendTimeIst: p.sendTimeIst ?? null,
      dateRange: p.dateRange ?? null,
      toEmails: p.toEmails ?? [],
      ccEmails: p.ccEmails ?? [],
      includeSummary: p.includeSummary ?? null,
      includeDetailed: p.includeDetailed ?? null,
      includeKeyAccount: p.includeKeyAccount ?? null,
      includeTraceableExport: p.includeTraceableExport ?? null,
      includeOpenCallsExport: p.includeOpenCallsExport ?? null,
      bodyInEmail: p.bodyInEmail ?? [],
      keyAccountsInBody: p.keyAccountsInBody ?? [],
    });
    const beforeSnap = prefsAuditSlice(current);
    const afterSnap = prefsAuditSlice(validated.merged);
