...
/** CSRF guard skips when Authorization is Bearer (same as the profile UI). */
function sendRequest(body: Record<string, unknown>) {
  return new Request('http://localhost/api/profile/mis-email/send', {
    method: 'POST',
    headers: {
...
describe('POST /api/profile/mis-email/send gates', () => {
  ...
  it('returns 403 when role lacks mis_email_send', async () => {
    ...
    const res = await POST(sendRequest({ preferences: { includeSummary: true } }));
