  ...
  if (loading) return (
    <div className="flex h-screen items-center justify-center bg-slate-50">
      <div className="flex flex-col items-center gap-3">
        ...
        <p className="text-sm font-bold text-slate-500 uppercase tracking-widest">Securing access...</p>
      </div>
    </div>
  );
    ...
    <div className="flex-1 flex flex-col h-full overflow-hidden bg-[#f8fafc]">
      ...
      <header className="bg-white border-b border-slate-200 flex-shrink-0">
        <div className="h-14 px-7 flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <h1 className="text-sm font-black tracking-tight uppercase text-slate-900">Roles & Access Control</h1>
          </div>
              ...
              <div key={role.id} className="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm hover:shadow-md transition-all group">
                <div className="flex justify-between items-start mb-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <h3 className="font-black text-slate-900 text-[15px] uppercase tracking-tight">{role.name}</h3>
                      <p className="text-[12px] text-slate-400 font-medium">{role.description || 'No description provided'}</p>
                    </div>
                  </div>
                ...
                <div className="space-y-2 mt-6">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {role.permissions && role.permissions.map((p: string) => (
                      <span key={p} className="px-2.5 py-1 bg-slate-50 border border-slate-100 text-slate-600 rounded-lg text-[10px] font-bold uppercase tracking-wide">
                        {p.replace(/_/g, ' ')}
                      </span>
                    ))}
      ...
      {showModal && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-4">
          ...
          <div className="bg-white rounded-[32px] shadow-2xl w-full max-w-2xl z-10 overflow-hidden animate-in zoom-in-95 slide-in-from-bottom-4 duration-300">
            <form onSubmit={handleSubmit}>
              <div className="px-8 py-6 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-[12px] text-slate-400 font-bold uppercase tracking-widest">Access Control Configuration</p>
                  </div>
                </div>
              ...
              <div className="p-8 space-y-8 max-h-[60vh] overflow-y-auto custom-scrollbar">
                <div className="grid grid-cols-1 gap-6">
                  <div className="space-y-2">
                    <label className="text-[11px] font-black text-slate-400 uppercase tracking-widest ml-1">Role Identifier</label>
                    <input
                      required
                      placeholder="e.g. Regional Manager"
                      className="w-full bg-slate-50 border border-slate-200 rounded-2xl py-3.5 px-5 text-[14px] font-bold text-slate-900 outline-none focus:ring-2 focus:ring-slate-100 focus:border-slate-300 transition-all shadow-inner"
                      value={formData.name}
                  ...
                  <div className="space-y-2">
                    ...
                    <textarea
                      ...
                      rows={2}
                      className="w-full bg-slate-50 border border-slate-200 rounded-2xl py-3.5 px-5 text-[14px] font-medium text-slate-600 outline-none focus:ring-2 focus:ring-slate-100 focus:border-slate-300 transition-all shadow-inner resize-none"
                ...
                <div className="space-y-4">
                  <div className="flex items-center justify-between">
                    <label className="text-[11px] font-black text-slate-400 uppercase tracking-widest ml-1">System Permissions</label>
                    <span className="text-[10px] font-bold text-slate-300 italic">{formData.permissionIds.length} selected</span>
                  </div>
                    ...
                    {allPermissions.map((p) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <div>
                            <p className={`text-[12px] font-bold uppercase tracking-tight ${isSelected ? 'text-white' : 'text-slate-900'}`}>
                              {p.name.replace(/_/g, ' ')}
                            </p>
              ...
              <div className="p-8 bg-slate-50/50 border-t border-slate-100 flex items-center justify-end gap-3">
                <button
                  ...
                  className="px-6 py-3 text-[13px] font-bold text-slate-500 hover:text-slate-900 transition-all uppercase tracking-widest"
                >
                  Cancel
                </button>
                <button
                  type="submit"
                  className="bg-slate-900 text-white px-8 py-3.5 rounded-2xl text-[13px] font-black uppercase tracking-widest hover:bg-slate-800 transition-all shadow-xl shadow-slate-200 active:scale-95"
                >
