...
export function DesktopView({
  ...
  timePeriod, dateRange, setTimePeriod, copyToClipboard,
  ...
  const [tempSelectedOfficeId, setTempSelectedOfficeId] = React.useState<string>(selectedOfficeId);
  ...
  React.useEffect(() => {
    if (showBranchDropdown) {
      setTempSelectedOfficeId(selectedOfficeId);
    }
  }, [showBranchDropdown, selectedOfficeId]);
  ...
  return (
    <div className="flex flex-col h-screen bg-[#f8fafc] overflow-hidden font-sans">
      ...
      <header className="flex-shrink-0 border-b border-slate-200/80 bg-white">
        ...
        <div className="px-7 h-12 flex items-center justify-between border-t border-[#e2e8f0] bg-white">
          <div className="flex items-center">
            <select
              value={activeTab}
              onChange={(e) => setActiveTab(e.target.value as 'all' | 'major' | 'minor')}
              className="h-8 pl-3 pr-8 bg-white border border-[#e2e8f0] rounded-lg text-[#475569] text-[12px] font-medium hover:border-slate-400 focus:outline-none shadow-sm cursor-pointer"
            >
              <option value="all">All Classification</option>
              <option value="major">Major Calls</option>
              <option value="minor">Minor Calls</option>
            </select>
          </div>
                ...
                {showBranchDropdown && (
                  <div className="absolute top-full left-0 mt-2 w-[300px] bg-white border border-slate-200 shadow-2xl rounded-xl z-[100] overflow-hidden animate-in fade-in zoom-in-95 duration-200">
                    ...
                    <div className="max-h-[300px] overflow-y-auto custom-scrollbar p-2">
                      <button
                        onClick={() => { setTempSelectedOfficeId(''); }}
                        className={`w-full text-left px-3 py-2 text-[11px] font-medium rounded-lg mb-1 transition-all ${!tempSelectedOfficeId ? 'bg-slate-900 text-white' : 'text-slate-500 hover:bg-slate-50'}`}
                      >
                        Global View
                      </button>
                      <BranchTree
                        offices={offices}
                        selectedIds={tempSelectedOfficeId ? [String(tempSelectedOfficeId)] : []}
                        setSelectedIds={(ids) => {
                          const id = ids[0] || '';
                          setTempSelectedOfficeId(id);
                        }}
                        single
                        search={branchSearch}
                      />
                    ...
                    <div className="p-2 bg-slate-50 border-t border-slate-100 flex items-center justify-between gap-2">
                      <button 
                        ...
                        className="px-3 py-1.5 text-[11px] font-medium text-rose-500 hover:bg-rose-50 rounded-lg transition-colors"
                      >
                        Clear Filter
                      </button>
                      <div className="flex items-center gap-2">
                        <button 
                          ...
                          className="px-3 py-1.5 text-[11px] font-medium text-slate-500 hover:bg-slate-200 rounded-lg transition-colors"
                        >
                          Cancel
                        </button>
                        <button 
                          onClick={() => {
                            setSelectedOfficeId(tempSelectedOfficeId);
                            setShowBranchDropdown(false);
                            setBranchSearch('');
                          }}
                          className="px-3 py-1.5 text-[11px] font-medium bg-slate-900 text-white hover:bg-slate-800 rounded-lg transition-colors shadow-sm"
                        >
                          Apply Filter
                        </button>
                      </div>
      ...
      <main className="flex-1 flex flex-col min-h-0 relative bg-slate-50/10">
        {loading && calls.length === 0 ? (
          ...
          <div className="flex-1 flex flex-col min-h-0 w-full p-6">
            <div className="flex-1 min-h-0 relative">
              {loading && calls.length > 0 && (
                <div className="absolute top-0 right-6 mt-2 mr-2 p-2 z-30 flex items-center gap-2">
                  <button
                    onClick={onStopSync}
            ...
            <div className="flex-shrink-0 mt-4 flex items-center justify-between border-t border-slate-100 pt-4">
