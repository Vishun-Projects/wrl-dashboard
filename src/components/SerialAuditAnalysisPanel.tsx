    ...
    <table className="w-full table-fixed border-collapse text-left text-[11px]">
      ...
      <tbody>
        {entries.map((entry, index) => {
          ...
          return (
            <tr key={`${entry.franchisee}-${entry.technician}-${rank}`} className="border-b border-slate-100">
    ...
    <section className="flex min-h-0 w-full min-w-0 flex-col overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm lg:max-h-full">
        ...
        <div className="flex items-start gap-2">
          ...
          <div className="min-w-0 flex-1">
            ...
            <p className="mt-0.5 text-[10px] leading-snug text-slate-500">
              {totalEntries > 0
                ? `${totalEntries.toLocaleString()} pairs · flagged serials`
      ...
      {loading && analysis.repeatCallCount === 0 && analysis.serialsWithDetails === 0 ? (
        ...
        <div className="min-h-0 flex-1 overflow-y-auto custom-scrollbar">
          <InvolvementTable entries={pagedEntries} rankOffset={rankOffset} />
        </div>
      )}
